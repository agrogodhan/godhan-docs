...
19. **ESP32 firmware BLE provisioning — written 2026-07-20, not hardware-tested.** Found while documenting §3.40's fix that no firmware in this repo implemented BLE at all (see `docs/IOT_DEVICE_DESIGN.md` §3.6.1 for the full writeup); `godhan iot docs/esp_32_firmware_skeleton.cpp` now has a full BLE GATT server matching the mobile app's real contract exactly (service `00001234-...`, the same 4 characteristics, `GODHAN_`-prefixed advertising name), NVS-backed persistence via `Preferences` so provisioning survives deep sleep/power cycles, a factory-reset trigger (hold the `GPIO0` boot button), and the previously-missing `POST /api/devices/register` call. Hand-reviewed only (brace/paren balance, a full logical read-through that caught and fixed one real concurrency risk — a blocking WiFi-connect attempt that was originally firing directly inside a BLE GATT write callback, moved out to the main polling loop instead) — **there is no way to compile or hardware-test this in the current environment**, so treat it as a strong starting point, not verified-working firmware. Needs: a real compile against the project's actual ESP32 Arduino core version (the exact `BLEDevice`/`BLECharacteristic` API surface, e.g. `BLEDevice::deinit()`'s signature, has shifted across core versions), then a real on-device pairing test against the mobile app.
